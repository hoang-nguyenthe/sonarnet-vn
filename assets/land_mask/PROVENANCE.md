...
Policy v2 excludes every detection whose georeferenced bounding box intersects mapped land or the band extending 500 metres seaward from the shoreline. Harbour and nearshore vessels are deliberately excluded too. The original model may still compute boxes there; the geographic post-filter removes them from the active result, map, review list and AIS demonstration. This is an engineering buffer, not a guaranteed accuracy estimate. Small islands, reclaimed land, shoreline change, SAR geolocation errors and radar false positives remain possible. The mask does not prove vessel presence, type or AIS identity.
